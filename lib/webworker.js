    ...
    function clearQueue() {
        process.nextTick(function () {
            if (msgQueue.length > 0) {
                console.log('[' + process.pid + ']', 'post', util.inspect(msgQueue[0]));
                postMessage(msgQueue[0][0], msgQueue[0][1]);
                msgQueue.shift();
                process.nextTick(clearQueue);
            }
        });
    }
